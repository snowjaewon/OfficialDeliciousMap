...
분할을 요구하며 실패한다. 추가형 이력만 예외로, 조각이 상한에 닿으면 앞 조각을 그대로 두고
`<이름>.002.<확장자>`부터 세 자리 번호를 붙인 다음 조각을 연다. 읽는 쪽은 번호 순으로 이어 읽고,
번호가 비었거나 같은 `(key, revision)`이 두 조각에 있으면 실패한다. 상한 CI는 후속 작업이다.
