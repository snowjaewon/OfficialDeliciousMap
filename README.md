...
저장하지 않고 `unsupported-format`으로 실패한다. 집행내역을 첨부 대신 HTML 표로 내는 게시판(울산
시청·중구·동구)만 스크래퍼가 `.html`을 선언하고, 그 쪽 응답 전체를 컨테이너 `html` 원본으로
받는다([ADR-0008](docs/adr/0008-declare-html-table-mappings.md)).
