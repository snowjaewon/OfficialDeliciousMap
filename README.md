...
컨테이너 `html` 원본으로 받는다. HTML 판정은 게시판 단위로 켜지므로 첨부 게시판이 200으로 준 오류
화면은 원본이 되지 않는다. 울산 HTML 표의 헤더 매핑·집행일은 [ADR-0008](
docs/adr/0008-declare-html-table-mappings.md)을 따른다.
...
짐작하지 않고 대상에서 뺀다. 게시일을 밝히지 않는 게시판(대구 수성구)은 제목이 밝힌 해와 기간으로
가르고, 제목에 해가 없으면 대상에서 뺀다. 게시일도 제목도 없는 원본은 가를 근거가 없어 대상으로 둔다.
하루치 집행내역을 날짜로 여는 게시판(울산 시청·동구)의 원본은 제목이 기간을 적지 않으므로 상세
키가 밝힌 그 날(`SourceRef.spent_on`)로 가른다.
...
- `headermap`: 표마다 공통 헤더 서명 캐시 → 원본별 답변 이력 → Gemini 순으로 매핑을 찾고,
  ...
  레지스트리가 헤더 매핑을 선언한 HTML 표 게시판은 캐시·모델을 거치지 않고 선언만 쓴다. 쪽의 표
  가운데 첫 행이 선언한 헤더와 같은 표 하나에 적용하며, 그런 표가 없거나 둘 이상이면 미해결이다(ADR-0008).
- `parse`: xls·xlsx(ISO Strict 포함)·PDF·HWPX·HTML 표 쪽을 읽는다. PDF는 괘선으로 나뉜 표마다,
  HWPX는 본문의 `<hp:tbl>`마다, HTML은 `<table>`마다 표 하나이며 한 칸이 쪼개진 표기와 병합한
  칸을 원본의 행·열 자리로 되돌린다. 모든 표가 통과한 원본만 레코드를 낸다. 집행일 열이 없는 표는
  상세 키가 밝힌 날을 집행일로 쓴다. 선언한 천원 표에 10,000(천원) 이상의 값이 있으면 원으로 적은
  행이므로 곱하지도 고쳐 읽지도 않고 그 원본을 `validation_failed`(`amount_unit`)로 남긴다.
...
같은 대화상자가 이번 제출이 남긴 미해결도 사유별로 적는다([제출 시점 기준](
docs/specs/header-mapping-fallback.md#제출-시점-기준)). 사람이 원본과 대조해 확정한 **원본 결함
확정**(`data/manual/<city>/sources.jsonl`의 `confirmed_by`가 찬 원본)은 원본 수·지출 후보 수·사유를
따로 적고, 대조가 없는 원본은 사유가 같아도 미해결 원본으로 센다. 읽지 못한 원본이 남았는데 확정한
원본이 하나도 없으면 `아직 없습니다`라고 밝혀, 대조를 마친 것처럼 읽히지 않게 한다. 판단 보류 레코드 수와 좌표를
확정하지 못한 식당 레코드 수도 사유별로 적는다. 세지 않은 값은 0으로 적지 않는다 — 원본별 보고가
없는 산출물은 미해결 줄 자체를 내지 않고, 후보 수를 모르는 원본이 섞이면 `지출 후보 수 알 수 없음`으로
적는다. 사유 표기는 장부 탭(`app.js`)과 같은 말을 쓴다.
