...
- 원본·정제 산출물·캐시를 다룰 때 [ADR-0001](docs/adr/0001-commit-refined-artifacts.md), 헤더 매핑·추출을 다룰 때 [ADR-0002](docs/adr/0002-ai-header-mapping.md)와 [폴백 정책](docs/specs/header-mapping-fallback.md), 지오코딩 판정 키·이력 분할을 다룰 때 [ADR-0003](docs/adr/0003-narrow-geocode-history-key.md), 누적 재게시 병합을 다룰 때 [ADR-0004](docs/adr/0004-merge-repeated-reposts.md), 판정 키가 레코드에서 담는 값을 다룰 때 [ADR-0005](docs/adr/0005-key-only-what-the-decision-reads.md), 독립 근거 없는 레코드의 채택을 다룰 때 [ADR-0009](docs/adr/0009-adopt-provider-cross-in-city.md)와 [ADR-0010](docs/adr/0010-adopt-single-provider-in-city.md), 후보 조회의 제공자 순서를 다룰 때 [ADR-0011](docs/adr/0011-ask-providers-in-order.md), HTML 표 게시판의 원본·헤더 매핑·집행일을 다룰 때 [ADR-0008](docs/adr/0008-declare-html-table-mappings.md)을 읽는다.
