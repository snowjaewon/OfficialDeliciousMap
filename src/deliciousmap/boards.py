...
class Posting:
    ...
    # 게시글 주소. 첨부가 하나도 없는 게시글을 수집 기록에 남길 때 그 줄의 주소가 된다 —
    # 첨부가 있으면 첫 첨부의 `page_url`이 그 자리를 채운다.
    #
    # 본문까지 보고 첨부가 없음을 확인한 게시글에만 채운다. 본문을 열지 않고 넘긴 게시글
    # (이미 끝냈거나 기간 밖이거나 업무추진비 글이 아닌 것)은 비워 둔다 — 채우면 넘긴 게시글과
    # 첨부 없는 게시글이 같은 모양이 되어 수집이 둘을 가르지 못한다(#212).
    #
    # 화면 자체가 원본인 게시판은 받는 화면이 곧 원본이라 첨부 없는 게시글이 없다. 그런
    # 게시판의 스크래퍼는 이 칸을 채우지 않는다.
    url: str = ""
