...
# 지역검색이 `부산동구청`으로 적어 이름이 정확히 같지는 않다. 주소·좌표가 동구청
# 소재지(구청로 1)를 가리키는 것을 보고 받았다. 남구·강서구도 같은 사유다.
...
# 지역검색 표기가 `부산광역시남구청`이다. 이름만 물으면 인천 미추홀·전남광주·울산 남구청과
# 함께 넷째로 나오고, 주소·좌표가 남구청 소재지(못골로 19)를 가리킨다.
NAM_HALL = Hall(35.1365769, 129.084163)  # 남구 못골로 19
...
# 지역검색 표기가 `부산광역시 강서구청`이다. 이름만 물으면 서울 강서구청만 나와 `부산강서구청`
# 으로 물었고, 주소·좌표가 강서구청 소재지(낙동북로 477)를 가리킨다.
GANGSEO_HALL = Hall(35.2122178, 128.98045)  # 강서구 낙동북로 477
...
CITY = City(
    ...
    # 주소를 밝힌 후보 32,943건 가운데 6,226건이 이 접두로 시작하고, 두 제공자가 함께 쓴다
    # (인허가 3,293·네이버 2,933). 다른 부산 표기는 없다. 세는 법은
    # `docs/validation/issue-142.md` 7절에 있다.
