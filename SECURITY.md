...
| 키 | 성격 | 두는 곳 |
| --- | --- | --- |
| 네이버 검색 API Client ID / Secret (지오코딩) | **비밀** | 개발자 PC의 `.env` 만. CI는 쓰지 않는다(정제 산출물이 커밋되므로). |
| Gemini API 키 (헤더 매핑·비식당 판별) | **비밀** | 개발자 PC의 `.env` 만. CI는 쓰지 않는다. |
...
5. **정제 산출물에 개인정보가 섞이지 않게 한다.** 레코드에는 기관·부서·상호·금액·집행일만 남기고, 참석자 이름·인원 같은 열은 파싱 단계에서 버린다.

## 새면 어떻게 하나

1. 해당 키를 즉시 콘솔에서 폐기하고 재발급한다(네이버 클라우드 콘솔, Google AI Studio).
2. 새 키를 대면으로 전달한다.
