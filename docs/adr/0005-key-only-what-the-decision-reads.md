...
- 같은 원칙을 **산출물 의존성 키**에도 적용했다([#190](https://github.com/snowjaewon/OfficialDeliciousMap/issues/190), 2026-09-17). `data/manual/<city>/`의 검토 파일 4종(`merchants`·`classify`·`restore`·`geocode`)은 파일 통째 SHA-256 대신 `storage.review_digest`가 `contracts.REVIEW_EVIDENCE_FIELDS`(`evidence`·`references`)를 뺀 판정 필드를 정렬해 묶은 해시로 `dependencies`에 들어간다. 근거 문구·참조·줄 순서만 고친 편집은 산출물을 낡게 하지 않는다. 적용 규칙(`normalized()` 등)의 변경은 여전히 각 정책 버전이 잡는다.
