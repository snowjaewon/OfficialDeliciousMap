...
지원한다. 조각이 20MB에 닿으면 앞 조각을 그대로 두고 `geocode-history-v2.002.jsonl`부터 세 자리
번호를 붙인 다음 조각을 연다. 읽는 쪽은 번호 순으로 이어 읽으며, 조각 번호가 비었거나 같은
`(key, revision)`이 두 조각에 있으면 실패한다. 커밋된 조각은 다시 쓰지 않는다.
