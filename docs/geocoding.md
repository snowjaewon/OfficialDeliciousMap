...
`data/manual/<city>/restore.jsonl`의 확정 복원명이 있으면 그 이름으로, 없으면 원본 표기에서
이름 없는 동행 업소의 꼬리말을 뗀 이름으로 요청한다(아래). 사람 확인이 규칙보다 앞선다.
그렇게 정한 요청 이름에 업소 둘 이상이 적혀 있으면 구분자로 나눈 이름으로도 조회해 캐시에
쌓는다. 그 후보는 판정에 넣지 않는다 — 나누는 것은 사람 확인뿐이다([상호 복원](restoration.md#상호-가르기--합쳐-적은-상호를-업소마다)).
요청 맥락의 도시·기관은 질의에 넣지 않는다. 도시는 자료를 공개한 기관의 단위일 뿐이므로
기관 도시 밖 후보를 거르거나 기관 도시 안 동명이 업소로 바꾸지 않는다.

### 이름 없는 동행 업소의 꼬리말

상호 칸 끝의 `외 1`은 둘째 업소의 이름을 적지 않고 수만 밝힌다. 그 표기를 통째로 조회하면
제공자에 그런 업소가 없어 후보가 0건이 된다. 조회와 근거 대조는 꼬리말을 뗀 첫 업소의 이름을
쓰고, 레코드의 원본 표기는 바꾸지 않는다. 규칙은 `merchants` 모듈 하나가 소유한다.

| 대상 | 예 | 조회 이름 | 이름 없는 업소 |
| --- | --- | --- | --- |
| `외` + 수 | `스타벅스 외 1` | `스타벅스` | 1곳 |
| 세는 낱말(`개소`·`곳`·`명`·`개`) | `명태어장 외 1곳` | `명태어장` | 1곳 |
| 공백 없는 표기 | `순창민물매운탕외1개소` | `순창민물매운탕` | 1곳 |
| 수도 없는 맨끝 `외` | `낙지촌 외` | `낙지촌` | 알 수 없음 |

꼬리말은 상호 **끝**에서만 뗀다. 이름 가운데의 `외`(`외갓집 순두부 본점`)는 그대로 둔다.
수도 적지 않은 맨끝 `외`는 앞의 공백을 요구한다 — `광주 농협로컬푸드직외`처럼 공백이 없으면
낱말이 잘린 것인지 꼬리말인지 가를 근거가 없다. 떼면 이름이 하나도 남지 않는 표기(`외 1`)도
그대로 둔다.

수를 적은 꼬리말은 그 반대로 앞의 공백을 요구하지 않는다. `카페말바우외 1`을 떼려면 그래야
하고, 그 대가로 `외`로 끝나는 낱말 뒤에 수가 오면 잘린다(`치킨플러스 해외 1` → `치킨플러스 해`).
광주 249건에는 그런 표기가 없다. 잘린 이름은 후보를 못 얻거나 근거와 어긋나 미확정으로 남는다 —
자동 채택은 잘린 이름이 후보의 앞이나 뒤에 3자 이상 그대로 붙을 때만 같은 업소로 본다(아래
상호 포함 일치). 이 선택은 `tests/test_merchants.py`가 고정한다.

이름을 다 적고 구분자로 나눈 합쳐 적은 상호는 가르는 일이므로 여기서 떼지 않는다
([#117](https://github.com/snowjaewon/OfficialDeliciousMap/issues/117)).

이름 없는 업소는 조회할 이름이 없어 영영 마커가 되지 못한다. 그 수는 레코드마다 장부
(`records.json`의 `unnamed_companions`)에, 합계는 화면의 자료 범위에 남는다. 원본이 수를
적지 않았으면 몇 곳인지 알 수 없으므로 합계에 넣지 않고 그 레코드 수를 따로 낸다.
업소 둘에 걸친 지출의 금액은 [ADR-0007](adr/0007-do-not-split-unallocated-amounts.md)을 따라
배분하지 않는다. 실측과 결과는 [이슈 #127 검증](validation/issue-127.md)에 있다.
...
실행 중 새로 얻은 조회는 옆의 대기 파일 `geocode-lookup-v1.pending.jsonl`에 한 줄씩 덧붙이고,
조회를 마칠 때 한 번 정렬해 캐시에 합친다. 조회마다 캐시를 통째로 다시 쓰지 않는다([#181](https://github.com/snowjaewon/OfficialDeliciousMap/issues/181)).
끊긴 실행이 남긴 대기 파일은 다음 실행이 읽어 재사용하고, 그 실행이 조회를 마칠 때 함께 합치므로 받은
조회를 잃지 않는다. 쓰다 끊긴 끝 줄 하나만 버린다. 대기 파일은 커밋하지 않는다. 업종 캐시도 같은 규칙이다.
...
업종은 업소 확인의 근거가 아니라 표시용 정보다. `geocode`는 판정을 저장한 뒤 확정 업소마다
그 업소를 확정한 후보의 업종을 얻는다. 확정한 후보는 마커의 `coordinate_source`를 고르는 규칙과 같이
첫 레코드의 판정에서 고른다. 그 후보를 찾은 조회를 같은 제공자에게 같은 질의로 다시 묻고, 응답에서
확정한 후보와 출처(`source_id`)가 같은 항목의 업종 원문만 쓴다 — 네이버는 `category`, 인허가는
업태구분명(`BZSTAT_SE_NM`)이다. 같은 실행에서 방금 받은 응답은 다시 요청하지 않는다.

결과는 `category-lookup-v1.jsonl`에 제공자·해석 버전·질의어의 해시 키로 쌓고 build만 읽는다.
판정 키·판정 이력·조회 캐시와 `geocode.json`의 의존성에는 들어가지 않는다. 담당자가 준 후보처럼
조회로 찾지 않은 후보, 다시 물었을 때 그 후보가 없는 업소는 업종을 `미상`으로 둔다. 조회가 실패하면
`geocode`는 판정을 저장한 채 `lookup-failed`로 끝나고, 기록된 실패는 `--retry-failed` 전까지 다시 묻지
않는다. 실측과 결정은 [이슈 #96 검증](validation/issue-96.md)에 있다.

## 판정과 식별자

담당자가 **독립 근거**(`facts`)를 적은 레코드는 그 근거의 상호·지점·주소가 일치하고 업소 하나를
특정할 때만 채택한다. 한 제공자가 같은 상호·지점·주소의 후보를 여럿 주면 서로 다른 업소일 수 있으므로
`ambiguous`로 남긴다. 비교 정규화는 Unicode NFC·대소문자·공백 정리에 한정한다.

독립 근거가 없는 레코드는 **도시 안 후보로 채택**한다([ADR-0009](adr/0009-adopt-provider-cross-in-city.md),
[ADR-0010](adr/0010-adopt-single-provider-in-city.md)). 후보 주소가 `City.address_prefixes`로 시작하고
레코드의 상호와 **상호 포함 일치**면 제공자가 하나여도 채택한다. 상호 포함 일치는 꼬리말·법인 표기·괄호
안·공백·기호를 뗀 뼈대가 같거나, 짧은 쪽이 3자 이상이고 긴 쪽의 앞이나 뒤에 그대로 붙는 것이다. 중간
겹침과 2자 접두·접미는 다른 업소로 본다. 유사도·편집 거리·주소 추정은 쓰지 않는다. 접두를 적지 않은
도시는 이 경로로 채택하지 않으며, 업소 둘 이상으로 읽히는 상호는 뼈대가 글자까지 같을 때만 받는다(#117).

좌표가 200 m 안인 후보는 한 장소로 묶는다. 인허가 좌표는 중부원점TM을 변환한 값이라 같은 건물도
수십 m 어긋나기 때문이다(광주 실측 최대 166 m). 같은 상호가 도시 안 여러 장소에 있으면 상호 겹침이 더
긴 곳을 먼저 고르고, 같으면 레코드가 적은 기관의 **청사**(`Organization.hall`)에 가까운 곳을 고른다.
청사를 적지 않은 기관은 여러 곳 가운데 하나를 고르지 않고 `conflicting_evidence`로 남긴다.

마커의 좌표와 표기는 한 후보에서 함께 나오며 `naver` → `local` → `license` 순으로 고른다. 성공한 판정의
`evidence`에는 무엇으로 확정했는지가 남는다 — `provider-cross license+naver nearest-hall 1234m`,
`single-provider naver nearest-hall 1234m`, 독립 근거는 `name-branch-address-agreement`다.
레코드마다 채택한 제공자가 달라 같은 업소가 갈리면, 상호 포함 일치이고 **정본에서** 200 m 안인 업소를
사람 확인·독립 근거 → 네이버 → 인허가 순의 한 곳으로 합치고 합쳐진 쪽의 `evidence`에
`merged-into naver`를 남긴다. 사람 확인과 독립 근거로 확정한 레코드는 채택보다 단단해 다시 쓰지 않는다.
잘린 원본과 전체 상호가 다르면 사람 확인 전까지 복원명을 확정하지 않는다.
확정 복원명이 있으면 원본 표기 대신 그 이름을, 없으면 꼬리말을 뗀 이름을 근거와 대조한다.
같은 업소의 좌표 근거가 레코드 사이에서 충돌하면 모두 보류한다.
...
| `lookup_key` | 판정이 레코드에서 읽는 값(`record_id`·`merchant`)·조회 결과·사람 확인·확정 복원명·도시 주소 접두·기관 청사 좌표·정책 해시. 동명이 업소 간 캐시 전파 방지 |
...
build는 금액 미상 방문과 이름 없는 동행 업소의 수를 함께 담은 v9다.
