...
실행 중 새로 얻은 조회는 옆의 보류 파일 `geocode-lookup-v1.pending.jsonl`에 한 줄씩 덧붙이고,
조회를 마칠 때 한 번 정렬해 캐시에 합친다. 조회마다 캐시를 통째로 다시 쓰지 않는다([#181](https://github.com/snowjaewon/OfficialDeliciousMap/issues/181)).
끊긴 실행이 남긴 보류 파일은 다음 실행이 시작할 때 먼저 합치므로 받은 조회를 잃지 않는다. 쓰다 끊긴
끝 줄 하나만 버린다. 보류 파일은 커밋하지 않는다. 업종 캐시도 같은 규칙이다.
