...
광주 249건에는 그런 표기가 없다. 잘린 이름은 후보를 못 얻거나 근거와 어긋나 미확정으로 남을 뿐
다른 업소를 확정하지는 않는다 — 자동 채택은 상호·지점·주소의 독립 근거를 그대로 요구한다.
이 선택은 `tests/test_merchants.py`가 고정한다.
...
담당자가 독립 근거를 적지 않은 레코드는 제공자 합의로만 채택한다([ADR-0009](adr/0009-provider-cross-as-independent-root.md)):
네이버와 인허가가 레코드의 상호(확정 복원명이 있으면 그 이름, 없으면 꼬리말을 뗀 이름)와 같은
이름으로 같은 지점·주소를 가리키고, 그 주소가 레지스트리 `City.address_prefixes` 가운데 하나로
시작해야 한다. 접두를 적지 않은 도시는 합의로 채택하지 않는다. 합의가 두 업소에 서면
`conflicting_evidence`, 합의가 없으면 `insufficient_evidence`다. 독립 근거가 있으면 그 근거가 먼저
판정하고 합의는 보지 않는다.
한 제공자가 같은 상호·지점·주소의 후보를 여럿 주면 서로 다른 업소일 수 있으므로 `ambiguous`로 남긴다.
서로 다른 제공자가 같은 상호·지점·주소를 가리키면 근거가 겹친 것으로 보고 한 업소로 다룬다.
이때 좌표를 준 곳이 하나면 그 좌표로 보강하고, 좌표가 둘 이상이면 서로 200 m 안일 때만 같은
업소로 본다(`identity.COORDINATE_TOLERANCE_M`). 마커에는 네이버·로컬·인허가 순으로 앞선 제공자의
좌표를 쓴다 — 네이버는 WGS84 원값이고 인허가는 중부원점TM을 변환한 값이라 같은 건물도
수 m~수십 m 어긋난다. 200 m를 넘으면 `conflicting_evidence`로 남겨 사람이 후보 하나를 확정할
때까지 마커를 보류한다.
성공한 판정의 `evidence`에는 근거가 겹친 출처를 함께 적는다(`... license+naver`). 독립 근거로
채택하면 `name-branch-address-agreement`, 제공자 합의로 채택하면 `provider-cross`로 시작한다.
...
| `lookup_key` | 판정이 레코드에서 읽는 값(`record_id`·`merchant`)·조회 결과·사람 확인·확정 복원명·도시의 주소 접두·정책 해시. 동명이 업소 간 캐시 전파 방지 |
...
| reason | 결과 |
| --- | --- |
| `matched`, `human_confirmed` | 독립 근거 일치·제공자 합의 또는 범위를 지정한 사람 확인으로 좌표 채택 |
| `missing_address`, `unknown_branch` | 독립 근거는 있으나 주소·지점 칸이 빔 |
| `insufficient_evidence` | 독립 근거도 제공자 합의도 없음. 후보 하나가 스스로 밝힌 주소는 근거가 아니다 |
...
독립 근거도 제공자 합의도 없어 `insufficient_evidence`로 남는 레코드는 이 경로로만 마커가 된다.
