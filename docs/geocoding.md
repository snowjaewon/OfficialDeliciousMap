...
| 오류 | 통신 실패와 `resultCode`가 성공(`"0"`)이 아닌 응답은 `unavailable`, 해석 불가는 `invalid_response`. 원문·상태 코드·키는 남기지 않는다 |
...
실제 키로 확인한 것(#73). 성공은 `resultCode` `"0"`·`resultMsg` `"정상"`이며 HTTP 상태 코드가
아니다. 공백이 든 질의어도 그대로 동작한다(`명가원 한정식` → 일반음식점 3건). 이 서비스는 0건과
한 건을 모두 `items` 객체 안의 `item` 목록으로 주었고, 목록 없는 객체 하나는 관찰하지 않았다.
`BPLC_NM`·`ROAD_NM_ADDR`·`LOTNO_ADDR`·`CRD_INFO_X/Y`·`MNG_NO`는 모두 있었다.
...
네이버는 아직 합성 응답으로만 검증했다([이슈 #39 검증](validation/issue-39.md)).
인허가는 광주 1,071개 질의를 실제 키로 조회했다([이슈 #73 검증](validation/issue-73.md)).
[이슈 #40 검증](validation/issue-40.md)의 제한 가운데 실제 키가 필요했던 것은 그 기록이 대신한다.
...
| `lookup_key` | 판정이 레코드에서 읽는 값(`record_id`·`merchant`)·조회 결과·사람 확인·확정 복원명·정책 해시. 동명이 업소 간 캐시 전파 방지 |
...
같은 기준을 광주 5개 자치구에 적용한 기록은 [이슈 #99 검증](validation/issue-99.md)에 있다.
거기서 `청사 생활권`은 구청의 관할인 구 단위로 읽었고(2026-09-13 사용자 결정), 방문 문턱과
정확 일치 조건은 그대로다. 이 기준으로 보류된 상호의 지점·전체 이름을 기관의 다른 공개자료가 적은 표기로
가린 기준은 [이슈 #74 검증](validation/issue-74.md)에 있다. 거기서는 그 표기가 가리키는 곳이 원본 표기의
조회에 없어 이미 캐시에 있던 그 표기의 조회 결과를 후보 파일로 공급했다. 판단 보류로 남던 상호를 사람
보정으로 식당으로 옮긴 뒤 새로 조회해 같은 기준을 적용한 기록은 [이슈 #122 검증](validation/issue-122.md)에 있다.
...
재사용 키(`lookup_key`)에는 판정이 실제로 읽는 것만 넣는다. 그 레코드에서 판정이 읽는 값, 적용한
후보 조회 결과, 그 레코드의 사람 확인과 확정 복원명, 그리고 판정 정책 키다. 선행 산출물·후보 파일·
조회 캐시·검토 파일의 **파일 해시는 넣지 않는다**. 판정이 그 파일들에서 실제로 읽은 값은 이미 키
안에 값으로 들어 있고, 파일 해시를 묶으면 판정이 하나도 바뀌지 않은 재실행이 모든 키를 갈아 이력을
통째로 다시 쌓기 때문이다([ADR-0003](adr/0003-narrow-geocode-history-key.md)). 같은 이유로 레코드도
**통째로 넣지 않는다**. 판정이 읽는 칸은 `record_id`와 `merchant`뿐이며, 그 목록은
`identity.KEYED_RECORD_FIELDS`에 두어 레코드 계약이 늘어도 키가 조용히 바뀌지 않게 한다
([ADR-0005](adr/0005-key-only-what-the-decision-reads.md)). 산출물 단위의 낡음은 envelope의
...
`geocode.json`도 상한을 넘으면 이력과 같은 번호 규칙으로 `geocode.002.json`부터 조각을 연다.
조각마다 같은 봉투 머리(`schema_version`·`city`·`org`·`dependencies`)를 되풀이하고 `results`만
나눈다. 읽는 쪽은 번호 순으로 이어 읽으며, 봉투 머리가 다른 조각이 섞여 있으면 실패한다.
낡음 검사는 조각을 모두 해시하므로 뒤 조각만 바뀐 변경도 `closure`·`build`가 잡는다.
쓸 조각은 이력에 손대기 전에 모두 만들어 크기를 확인한다. 결과 하나가 혼자 상한을 넘으면
나눌 자리가 없으므로 거부한다. 나눌 목록 칸을 정하지 않은 다른 단계도 넘는 순간 거부한다.
