...
### 대상 기간(2026년 상반기)에 드는 제외 줄 — 1줄

29줄을 원본의 `연도`·`월` 칸으로 가르면 2026년 상반기는 `R365` 하나이고 나머지 28줄은
2021~2025년이다(`amount_krw` 6줄은 모두 2022년 4~5월, `spent_on` 18줄은 사용일자를 읽지 못해
연도·월 칸으로만 가른 것).

| 행 | 사용일자 | 사용장소 | 사용목적 | 금액 | 사유 |
| --- | --- | --- | --- | --- | --- |
| R365 | 2026. 4. 26. | (빈칸) | 몽골 바가항가이구 방문단 환송연 | 258,000 | **장소 미표기** — 산출물 사유 `table1:R365 merchant` |

2026-09-17 사용자 결정: 이 줄은 장소 미표기로 기록하고 데이터에서 제외한다. 빈 상호를
사용목적으로 대신 채우지 않는다(폴백 정책). 그래서 기장군 2026년 상반기는 레코드 421건 + 제외
1줄 = 422줄이 표에 실린 전부다.
