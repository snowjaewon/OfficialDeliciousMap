...
### 실패한 게시판 — 중랑 하나였고 #154가 풀었다

중랑구 `seoul-jungnang/expenses`만 사유가 달렸었다: `service-unavailable`.
...
흡수했지만, 원본을 함께 받는 실제 수집은 이 장부를 낼 때까지 열두 번 시도해 한 번도 순회를 마치지
못했다(#154 본문은 그 뒤 한 번을 더해 열세 번으로 적었다).
...
그 쪽부터 잇게 했다. 2026-09-17 재수집의 실행 기록과 위 중랑 줄이 바뀐 근거는
[`issue-154.md`](issue-154.md)의 "서울 담당 PC의 재수집"에 있고 여기 옮겨 적지 않는다.
...
| 중랑 | 목록 한 쪽에 3.3초가 걸리고 그 사이 읽기가 끊긴다 | 열두 번 시도해도 순회를 마치지 못했다. [#154](https://github.com/snowjaewon/OfficialDeliciousMap/issues/154)가 순회 재개를 더해 풀었다 |
