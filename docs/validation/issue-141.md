...
| 중랑구 `seoul-jungnang` | 547 | 581 | ole2 10 · ooxml 31 · pdf 540 | 7,981 | 0 |
...
받지 않은 게시글·걸러 낸 게시글은 게시판을 끝까지 훑은 기관의 값이다. 중랑은 이 장부를 처음
낼 때 순회를 마치지 못해 두 값이 0이었으나, [#154](https://github.com/snowjaewon/OfficialDeliciousMap/issues/154)
재수집이 853쪽을 끝까지 훑어 다른 기관과 같은 단위의 값으로 바꿨다(아래 "실패한 게시판").

컨테이너 합계: `html 3,543 · jpeg 24 · ole2 705 · ooxml 1,942 · pdf 9,094 · png 3 · zip 2`
= 원본 15,313건, 게시글 15,055건, 받지 않은 게시글 164,413건,
걸러 낸 게시글 427건.

### 실패한 게시판

**해결됐다.** 중랑구 `seoul-jungnang/expenses`만 사유가 달렸었다: `service-unavailable`.
[#154](https://github.com/snowjaewon/OfficialDeliciousMap/issues/154)가 순회 재개로 풀었고, 위 표의
중랑 줄은 순회를 마친 값이다. 지금 25개 기관 어느 `fetch.json`에도 `collection failures`가 없다.

이 호스트는 목록 한 쪽에 3.3초가 걸리고(853쪽 ≈ 47분) 그 사이 읽기가 간헐적으로 끊긴다.
프로젝트 UA로 목록만 훑는 별도 측정에서는 853쪽을 2,522초에 완주했고 재시도 4회가 그 끊김을
흡수했지만, 원본을 함께 받는 실제 수집은 열세 번 시도해 한 번도 순회를 마치지 못했다.
그동안 `uncollected_postings`가 0이었던 것은 받지 않은 게시글이 없어서가 아니라 순회가 2026년
구간을 지나 끝까지 가지 못했기 때문이다.

#154는 재시도 횟수·호스트 간격을 바꾸는 대신 마지막으로 끝낸 쪽을 원본과 함께 기록해 다음 실행이
그 쪽부터 잇게 했다(`docs/validation/issue-154.md`). 2026-09-17 재수집은 두 번 실행해 끝났다 —
첫 실행이 264쪽에서 끊기고 이어 간 실행이 853쪽까지 갔다. 원본은 574건에서 581건으로 늘었는데,
574건은 전부 그대로이고 2026-09-15~17에 새로 올라온 2026년 게시글 7건이 더해진 것이다
(`source_hash` 집합 비교).
...
이 절의 수는 2026-09-14 장부(원본 15,306건) 위에서 한 번 읽은 것이고 다시 읽지 않았다. 위 표의
원본 합계는 #154 재수집으로 15,313건이 됐으므로, 늘어난 중랑 7건은 이 절의 어느 수에도 들어 있지
않다.
...
남긴다. 지금은 37건이 원본으로는 수집돼 있고 대상 선별에서만 빠진다. 이후
[#155](issue-155.md)에서 37건 중 32건을 읽고 5건을 이유와 함께 미선언으로 남겼다.
...
| 중랑 | 목록 한 쪽에 3.3초가 걸리고 그 사이 읽기가 끊긴다 | 열세 번 시도해도 순회를 마치지 못했다. [#154](https://github.com/snowjaewon/OfficialDeliciousMap/issues/154)가 순회 재개를 더해 풀었다 |
