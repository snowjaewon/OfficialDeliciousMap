...
| 광진구 `seoul-gwangjin` | 442 | 441 | ooxml 91 · pdf 350 | 5,249 | 0 |
...
| 구로구 `seoul-guro` | 555 | 555 | ole2 185 · pdf 370 | 10,306 | 0 |
...
**받지 못한 원본(`missing`)은 24개 기관 전부에서 0건이고, 사유가 달린 빈 결과(`empty_reason`)도 없다.**

컨테이너 합계: `html 3,543 · pdf 8,551 · ooxml 1,911 · ole2 695 · jpeg 24 · png 3 · zip 2`
= 원본 14,729건, 게시글 14,505건, 받지 않은 게시글 156,604건, 걸러 낸 게시글 256건.
...
수집한 원본 14,729건의 제목을 `period.exclusion`으로 읽었다.

| 판정 | 수 |
| --- | --- |
| 대상(`None`) | 11,214 |
| 대상 기간 밖(`declared_out_of_range`) | 3,409 |
| 게시일이 대상 연도 밖(`posted_out_of_range`) | 63 |
| **기간 미선언(`undeclared_in_year`)** | **43** |

`period.DECLARATION`에 더한 표기는 없다. 43건은 새 표기가 아니라 아래 네 갈래이고, 짐작으로 읽지
않고 기간 미선언으로 남긴다.

| 갈래 | 수 | 실측 예 |
| --- | --- | --- |
| 해를 `년` 없이 적고 달을 제목 끝 괄호에 둔다 | 28 | 구로 `2026 행정관리국 기관운영업무추진비 집행내역(4월)`, `2026 3월 자동차관리과시책추진업무추진비 공개` |
| 달만 적고 해가 없다 | 9 | 동작 `8월 업무추진비 사용내역 공개`, 중구 `12월 자치행정과 업무추진비 지출내역`, 양천 `7월 업무추진비 공개`, 성북 `민원여권과 1월 시책추진업무추진비 공개` |
| 기관이 잘못 적었다 | 2 | 성동 `2026월 1월 …`(`년`이 `월`), 성북 `…공개(206.4월)`(해가 세 자리) |
| 기간을 적지 않은 게시글 | 4 | 강남 `지방보조금으로 취득한 중요재산`, 강서 `안전교통국, 안전관리과 업무추진비 누락 내역 공개`, 서초 `방배2동 기관운영업무추진비 내역공개(현금)` |
...
| 중랑·동작·강동·강서·동대문·영등포·성북 | 긴 순회 중 간헐적으로 연결이 끊긴다 | 이어받기로 다시 실행한다. 이미 받은 원본은 다시 내려받지 않는다 |
