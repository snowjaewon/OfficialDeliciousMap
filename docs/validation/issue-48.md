...
- 운영 도메인 `officialdeliciousmap.pages.dev`에서의 지도 인증. 허용 목록 등록 여부를 이 세션에서
  확인하지 않았고, 배포·preview alias에서의 확인은
  [#15](https://github.com/snowjaewon/OfficialDeliciousMap/issues/15) 후속 구현의 범위다.
...
**허용 도메인과 인증.** 사용자가 네이버 클라우드 콘솔의 Maps 애플리케이션에 Web 서비스 URL
`http://127.0.0.1:8765`와 `http://localhost:8765`를 등록했다(사용자 보고, 콘솔 화면은 보지 않음).
두 주소 모두에서 실제 인증이 성공했다. SDK의 인증 오류 콘솔 메시지가 없었고 `navermap_authFailure`가
...
사용자가 `.env`의 키를 바꾼 뒤 `ncpKeyId`로 성공했다. 이 경우 `500`은 URL 문제가 아니라 Maps
애플리케이션 키로 인식되지 않는 값이라는 신호였다. 운영 도메인은 위 "확인하지 못한 것"에 남긴다.
...
1건씩 넣었다. `.env`에서는 `NAVER_MAP_*` 두 변수만 불러와 외부 조회 없이
`geocode`→`closure`→`build`를 실행했고, `python -m http.server 8765 --bind 127.0.0.1`로 띄웠다.
...
| 이동 제한(`maxBounds`) | 남동쪽(부산 방향)으로 큰 드래그 3회 | 남동 모서리 마커 부근에서 멈추고 부산 쪽으로 가지 않음 |
