...
- 운영 도메인 `officialdeliciousmap.pages.dev`에서의 실제 지도 인증. 허용 목록에는 등록했지만
  (아래 사용자 보고) 아직 배포가 없어 그 주소에서 인증이 성공하는지는 보지 못했다. 배포·preview
  alias에서의 확인은 [#15](https://github.com/snowjaewon/OfficialDeliciousMap/issues/15) 후속 구현의 범위다.
...
| `officialdeliciousmap.pages.dev` | 사용자 보고, `pages.dev` 전체 미등록 | 미확인(배포 전, #15) |
