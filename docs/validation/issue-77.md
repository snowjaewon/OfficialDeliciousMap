...
- 캐시 이름을 `deliciousmap-shell-v1`에서 `deliciousmap-shell-v2`로 올렸다. 새 워커가
  활성화되면 이전 버전 캐시를 삭제하고 `clients.claim()`으로 열린 페이지를 제어한다.
  셸 계약을 바꾸는 배포는 이 이름을 다시 올려 캐시를 비운다. 같은 버전의 자산 배포는
  한 번 오래된 셸이 보일 수 있지만, 백그라운드 갱신 뒤 다음 탐색부터 새 셸을 사용한다.
...
| `node --test tests/service_worker.test.js` | 4 passed |
