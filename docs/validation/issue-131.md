...
The HTML responses and originals used for reconnaissance were kept outside the
repository; no 원본 or `dist/` file is committed here.
...
| Ulsan city | `https://www.ulsan.go.kr/u/rep/transfer/director/list.ulsan?mId=001003002003000000` | 2,012 posts / 202 pages | inline transaction table; no 원본 endpoint |
| Ulsan city | `https://www.ulsan.go.kr/u/rep/transfer/ecnmy/list.ulsan?mId=001003002001000000` | 4,664 posts / 467 pages | inline transaction table; no 원본 endpoint |
| Jung-gu | `https://www.junggu.ulsan.kr/mayor/board/list.ulsan?boardId=BBS_0000006&listCel=1&listRow=10&menuCd=DOM_000000201005000000` | 8,046 rows / 805 pages | transaction rows; no 원본 endpoint |
...
| Dong-gu | `https://www.donggu.ulsan.kr/mayor/expense/list.do` | 4,032 rows / 404 pages | transaction rows; no 원본 endpoint |
...
The city transfer and Jung-gu/Dong-gu mayor boards are not silently treated as
failed originals: their measured contract is a published transaction table,
so the scraper preserves the posting/date/title and records no 원본.
...
year-only 원본 tally requires walking every declared page and receiving
...
and per-post 장부 entries; no 원본 or hashes are fabricated here.

## Fixture and format checks

`tests/test_ulsan.py` covers page traversal, malformed page metadata, direct
eGov and Jung-gu 원본 links, city encoded download arguments, Buk-gu and
Ulju detail boundaries, transaction boards without originals, `--org` selection,
