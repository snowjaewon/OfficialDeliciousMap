...
기관 단위 비율(각 `data/<city>/orgs/<slug>/build.json`의 마커 ÷ 레코드)은 울산 시청 0.21, 울산 구 0.32~0.57(레코드
33~198건), 광주 5개 구 0.23~0.29, 대전 4개 구 0.27~0.37(동구 0.30 · 중구 0.27 · 서구 0.37 · 대덕구 0.33)이다.
울산에서는 레코드가 몰린 시청이 구보다 비율이 낮고, 대전은 시청이 robots로 수집 보류라 레코드가 모두 자치구
것이다. 대전 도시 비율이 높은 것은 이 구성 차이와 맞는다. 광주 시청의 기관 산출물은 없어 비교하지 않았다.
유성구(레코드 30, 마커 5)는 구청장 게시판 하나뿐이고(#172) 대상 원본 5건 중 2건이 미해결이다.
...
- **LLM 비용.** 헤더 매핑은 표의 헤더 서명이 106종이라 1회 약 USD 0.0014로 USD 1 안팎(최악 약 2.8)으로 봤다.
  비식당 판별은 첫 parse 기준 캐시 미스 상호 2,575개 × USD 0.000185 ≈ USD 0.48로 봤다(키는 `classify.normalized`).
  최종 parse로 다시 세면 고유 상호 2,659개 중 `develop`의 `data/_shared/classify.jsonl` 적중 73개, 미스 2,586개다.
...
두 번째 고침으로 도시 미해결 원본이 100건에서 87건으로 줄었다. 유성구는 5건에서 2건으로 3건이 줄었다.
...
(원본 1,417 · 누락 0 · 받지 않은 게시글 11,702 · 걸러 낸 게시글 78, #172 기록과 같다). 원본이 있으면 `collect`가
보류 기관을 장부 경고에 넣지 않으므로 도시 `empty_reason`은 중구 291쪽 실패 하나이고, 시청 보류
(`collection held: daejeon-city=bot_blocked`)는 `orgs/daejeon-city/fetch.json`에 있다. `fetch --city`를 돌린
#172의 결과와 같은 모양이다.
