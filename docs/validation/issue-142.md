...
**이 문서는 `develop`(`f3522ca`)에 리베이스한 뒤 다시 낸 값이다.** 리베이스 전 이 브랜치는 열세
기관을 돌려 레코드 9,831건·마커 1,530개를 냈다. 그 사이 `develop`이 네 가지를 바꿨고 전제가
달라져 전 단계를 다시 돌렸다.
...
3. **공용 서명 캐시가 연도 근거를 잃던 버그를 고친다** (2026-09-17). 기관 실행과 도시 실행이
   같은 원본을 다르게 판정해 `geocode --org busan-nam`이 종료 코드 1로 끝났다. 원인과 고친
   자리는 8절에 있다. 파이프라인 코드 수정이며 부산만이 아니라 모든 도시의 다음 실행에 걸린다.
...
도시 `headermap`의 미해결 960건과 그 사유다. `parse`는 같은 960건에 `no_candidates` 104건을
더해 **1,064건**을 미해결로 남겼다.

| 사유 | 건수 | 뜻 |
| --- | ---: | --- |
| `unsupported_format` | 504 | OLE2 컨테이너인데 엑셀 통합문서가 아니다. HWP 5.0이며 현재 추출 경로가 없다 |
| `validation_failed` | 446 | 표는 읽었으나 매핑이 코드 검증을 통과하지 못했다 |
...
`validation_failed`는 리베이스 전 369건에서 446건으로 늘었다. 새 원본 529건이 들어온 몫이다.
...
## 5. parse·classify — 레코드 11,197건

대상 기간은 `2026-01-01/2026-06-30`이다. 도시 실행 기준으로 `parse`가 대상 원본 2,724건을 살펴
1,660건을 읽었고 지출 후보 17,371줄에서 레코드 11,197건을 냈다. 기간 밖으로 뺀 원본은
...
| **기관 합계** | **11,123** | **7,070** | **2,406** | **1,647** |
| **도시 실행** | **11,197** | **7,344** | **2,474** | **1,379** |

기관 합계와 도시가 레코드 74건 다른 것은 도시 `parse`가 기관 경계를 넘어 재게시를 합쳐 보기
때문이다. 식당·비식당·판단 보류가 갈리는 것은 비식당 판별이 상호 단위 캐시를 쓰기 때문이며,
도시 실행이 기관 실행 뒤에 돌아 캐시가 더 차 있었다.

**위 표의 "레코드"는 기관 산출물이 센 것이다.** 도시 `records.csv`를 기관별로 세면 수가 다른
기관이 셋 있다 — `busan-dongnae` 424(기관 387) · `busan-geumjeong` 417(382) ·
`busan-busanjin` 751(749). 나머지 열둘은 같다. 사이트의 `자료 범위`가 적는 건수는 이 **도시
쪽 수**다(12절). 두 수를 섞어 비교하지 않는다.
...
네이버가 확정 식당 7,344건 모두에 서고, 인허가는 네이버가 후보를 못 낸 982건에만 섰다. 곧
**뒤 제공자에게 묻는 비율이 13.4%** 다. 실제로 제공자에게 나간 요청은 이보다 훨씬 적다 —
...
도시 `headermap`이 매핑한 표 2,582개의 머리글을 실측했다. 장소류 열(`장소`·`가맹점`·`거래처`·
`집행처`·`업체`·`상호`)은 **2,261개 표(87.6%)** 가 가졌으나 그 칸은 대개 주소가 아니라 상호다.
...
| `busan-geumjeong` | 207 | 180 | 집행처 77 · 집행장소 38 |
...
| `busan-dongnae` | 51 | 49 | 장소 47 · 사용장소 (가맹점명) 2 |
| `busan-sasang` | 31 | 31 | 사용장소 (가맹점명) 11 · 사용장소 10 |
| **합계** | **2,582** | **2,261** | |
...
선언한 뒤 1,530개가 섰고, 이번 재실행에서 **1,721개**가 섰다.

| 사유 | 리베이스 전 | 이번 실행 |
| --- | ---: | ---: |
| `matched` | 3,864 | **4,228** |
| `insufficient_evidence` | 2,079 | 2,370 |
...
출력: 주소를 밝힌 후보 34,476건 가운데 `부산광역시` 6,666건이고 두 제공자가 함께 쓴다
(인허가 3,402 · 네이버 3,264). 다른 부산 표기는 없다. 나머지는 다른 시·도의 동명 업소다
(서울특별시 7,625 · 경기도 5,835 · 경상남도 2,733 · 전남광주통합특별시 1,652).

이 수는 캐시가 자라면 함께 자란다. 위 값은 이 문서를 쓴 시점의 것이다.

### 폐업 대조

`closure`는 확정 업소 1,721곳을 모두 `unknown`으로 남겼다. 지금 단계는 후보를 그대로 미확인으로
두며 이 이슈에서 바꾸지 않았다.

## 8. 기관별 종료 상태 (완료 기준 1)

**열다섯 기관 × 여섯 단계 아흔 개 실행이 모두 종료 코드 0**이고 도시 여섯 단계도 0이다.
아래는 마지막 실행의 값이다.
...
기관 마커 합계 1,858이 도시 1,721보다 많은 것은 같은 업소를 여러 기관이 각각 세기 때문이다.
...
3. **`geocode --org busan-nam` — `invalid-artifact`.** 남구 식당 레코드 216건 중 3건이 도시
   ...
   사용자 결정으로 고쳤다. `period.hinted_year`가 모델 지시문과 같은 근거(제목이 밝힌 지출
   기간의 해)를 코드로 읽고, `headermap._cached`가 캐시 판정을 되살릴 때 그 값을 넣는다. 캐시
   스키마는 그대로다 — 연도 근거는 여전히 원본의 것이고 캐시에 쌓이지 않는다. 고친 뒤 도시
   `headermap`이 **10분 59초에서 2분 44초로** 줄었다. 캐시가 실제로 쓰이기 시작한 것이다.
4. **`geocode --org busan-nam` — `lookup-failed`.** 3번을 고친 뒤 남은 실패다. 판정이 아니라
   확정 업소의 업종 조회가 실패했다. `--retry-failed`로 다시 돌려 0으로 끝냈다.

## 9. LLM 예산 (완료 기준 4)

리베이스 뒤 재실행이 `data/_shared/llm-budget.jsonl`에 더한 줄은 1,091이고, 예약 546 · 정산 545 ·
해제 0이다.
...
누적은 **$7.8321 / 한도 $15**다. 재실행 전 누적은 $6.5597이었다. 확인 명령(저장소 루트,
양쪽 공통):
...
모델은 `GEMINI_MODEL`에 선언된 `gemini-3.6-flash`다. 원본이 529건 늘었는데 호출이 528건에
그친 것은 8절의 캐시 수정이 도시 실행의 되묻기를 없앴기 때문이다.

## 10. 산출물 크기와 개인정보 (완료 기준 5)

`data/busan/` 아래 파일당 20MB 상한(ADR-0001)을 넘는 파일은 없다. 가장 큰 것은
`geocode-history-v2.002.jsonl` 15.8MB, `geocode.json` 14.6MB, `geocode-history-v2.jsonl` 13.9MB,
`geocode-lookup-v1.jsonl` 13.1MB다. 이력은 상한에 닿아 `.002`에 이어 `.003`(6.9MB)으로 한 조각
더 갈렸다.
...
| LLM 호출 수·누적 비용이 장부와 일치하고 USD 15 이내, 네이버 조회 수 기록 | 충족. $7.8321/15, 질의 8,326건 (6·9절) |
| `data/busan/` 산출물과 `dist/busan/`의 건수·크기, 파일 크기 상한·개인정보 검사 | 충족. 10절 |
| `build --city busan` 성공, 부산 페이지에서 장부·마커·보류 사유 확인 | 일부 충족. 마커 1,721과 보류 2곳 안내는 화면에서 봤다. `장부` 탭은 렌더러가 붙잡혀 화면으로 못 봤고 빌드 산출물(11,197건)로만 확인했다. 지도 타일은 지도 키의 출처 등록 문제로 뜨지 않는다 (12절) |
...
`http://localhost:8765/busan/`에서 **화면으로 본 것**.

- **마커·목록.** 목록 머리말이 `1,721곳 전체 · 1,708곳 현재 지도 영역`이고 방문 횟수 순으로
  `부자회관` 78회, `우나기만` 63회, `만성횟집` 57회가 이어진다. 전체 수가 빌드 산출물의
  `marker_count` 1,721과 같다.
...
**첫 로드는 옛 화면이었다.** 서비스 워커가 캐시한 이전 빌드가 떠서 보류를 넷으로 적었다.
강제 새로고침(`Ctrl+Shift+R`) 뒤 위의 값으로 바뀌었다. 배포 뒤 이용자 화면에도 같은 일이
생길 수 있다(14절).

**`장부` 탭은 화면으로 확인하지 못했다.** 탭을 누른 뒤 페이지가 45초 동안 `document_idle`에
닿지 않아 화면을 잡지 못했다. 콘솔은 그동안에도 읽혔으므로 탭이 죽은 것은 아니고 주 스레드가
붙잡힌 것이다. 레코드 9,831건이던 리베이스 전 실행에서는 이 탭이 떴다(14절).

화면 대신 빌드 산출물로 확인한 것.

- `dist/busan/records.json`의 레코드 11,197건 · `dist/busan/markers.json`의 마커 1,721개.
- `dist/busan/index.html`의 `자료 범위` 표가 `부산광역시 영도구 · 수집 보류 · 봇 차단`,
  `부산광역시 사하구 · 수집 보류 · 게시판 유실`을 적고, 보류가 풀린 둘은
  `부산광역시 남구 · 수집 완료 · 레코드 534건`, `부산광역시 강서구 · 수집 완료 · 레코드 761건`
  으로 적는다.
...
- **`장부` 탭이 11,197건에서 화면을 붙잡는다.** 탭을 누른 뒤 45초 동안 `document_idle`에 닿지
  않았다(12절). 레코드 9,831건이던 리베이스 전에는 떴으므로 건수가 늘며 드러난 자리로 보이나,
  원인을 이 이슈에서 좁히지 않았다. 별도 이슈로 다룰 일이다.
...
- **HWP 5.0(OLE2) 504건.** 추출 경로가 없어 미해결로 남았다. 이슈의 작업 분류표가 다른 세션
  몫으로 나눈 일이다.
- **`validation_failed` 446건.** 표는 읽었으나 매핑이 코드 검증을 통과하지 못했다. 사유별로 더
  갈라 보지 않았다.
- **`insufficient_evidence` 2,370건.** 도시 안에서 상호가 맞는 후보를 찾지 못했거나 여러 곳이라
  고르지 못한 레코드다. 사람이 확정하는 대량 작업은 이슈의 제외 범위다.
- **`busan-sasang` 훑기 중단과 보류 두 기관.** #140 계열이며 이 이슈에서 다시 시도하지 않았다.
- **폐업 대조.** 확정 업소 1,721곳이 모두 `unknown`이다.
