...
| --- | --- |
| Android Chrome | 미확인 |
| iPhone Safari(iPhone 13) | 2026-09-13 사용자 확인. PR #103 미리보기(`pr-103.officialdeliciousmap.pages.dev`)를 홈 화면에 추가하면 아이콘이 핀 모양으로 나온다 |

같은 확인에서 나온 지도 화면 사용성 지적(목록 시트 끌기, 확대 컨트롤, 첫 화면 크기, 방문 횟수 복수 선택,
마커 선택·해제)은 이 이슈의 범위가 아니어서 후속 이슈로 나눴다.
...
- Android Chrome 실기기 확인이 남았다(위 표).
