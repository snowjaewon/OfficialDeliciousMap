...
원본이 줄여 적은 상호라도 전체 이름의 후보가 이미 조회 결과에 있으면 복원 없이 업소 확인으로 바로
확정한다. 확인한 상호가 마커의 확정 상호가 되므로 전체 이름은 그대로 남는다. 복원하면 같은 후보를
얻으려고 다시 조회하고 판별할 뿐이다. 복원은 전체 이름으로 다시 조회·판별하거나 근거와 대조해야 할 때 쓴다.
지점은 복원명에 붙이지 않는다. 업소 확인의 상호는 지점을 뺀 이름이라 복원명과 달라져 충돌한다.
지점은 업소 확인이 가린다([이슈 #74 검증](validation/issue-74.md)).
