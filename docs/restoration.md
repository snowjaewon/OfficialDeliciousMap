...
원본이 줄여 적은 상호도 업소 확인의 기준을 통과하고 전체 이름의 후보가 이미 조회 결과에 있으면
복원 없이 업소 확인으로 확정한다. 이름이 겹친다는 것만으로는 확정하지 않는다. 복원은 전체 이름으로
다시 조회·판별하거나 근거와 대조해야 할 때 쓰며, 지점은 복원명에 붙이지 않고 업소 확인이 가린다.
이유는 [이슈 #74 검증](validation/issue-74.md#상호-복원이-아니라-업소-확인으로-가는-이유)에 있다.

지점을 빼면 조회가 그 업소에 닿지 못하고, 근거 자료가 지점까지 붙인 이름을 **띄어쓰기 없는
사업장명 한 덩어리**로 싣는 경우만 예외다. 그때는 후보의 지점 칸이 비어 있어 복원명과 업소
확인의 상호가 어긋나지 않는다. 인허가의 영업상태를 사람 검토의 근거로 읽는 것도 여기에 든다 —
후보 계약은 그 값을 싣지 않으므로 판정 규칙이 아니라 검토 기록에만 남는다. 두 예외를 적용한
기록은 [이슈 #136 검증](validation/issue-136.md#결정을-벗어난-세-가지와-그-까닭)에 있다.
...
| `release` | 응답을 받지 못한 호출의 예약을 소진액에서 해제한 기록. 금액은 0이다 |

누적액은 `prior_usage` + 정산액 + 아직 종결되지 않은 예약액이다. 여기에 새 요청의 상한을 더해
한도를 넘으면 호출하지 않는다. `prior_usage` 줄이 없으면 남은 잔액을 가정하지 않고
`unknown_prior_usage`로 보류한다. 응답을 받지 못한 호출(`unavailable`)은 `release`를 추가해
예약을 소진액에서 제외한다. 응답을 받았는데 사용량이 없거나 호출이 예외로 끝나면 과금됐을 수 있으므로
예약을 유지한다. 장부의 기존 줄은 고치거나 지우지 않는다.
