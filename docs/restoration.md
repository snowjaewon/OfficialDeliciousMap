...
지정하지 않은 건에는 호출이 0회다. 지정했더라도 이미 확정 복원명·업소 확인이 있거나 판정이
성공한 건, 후보가 없는 건은 호출하지 않고 `not_unresolved`·`no_candidates`로 남긴다.
...
헤더 매핑·비식당 판별·전량 추출 폴백·후보 비교가 프로젝트 전체 누적 **USD 15** 하나를 나눠 쓴다.
[폴백 정책](specs/header-mapping-fallback.md)이 기준이며 날짜·월·도시·실행마다 초기화하지 않는다.
집행은 `budget` 모듈 한 곳이 소유한다. 호출자는 잔액을 따로 계산하거나 장부를 직접 고치지 않는다.
...
한도를 넘으면 호출하지 않는다. `prior_usage` 줄이 없으면 잔액을 15달러로 가정하지 않고
`unknown_prior_usage`로 보류한다. 사용량을 확인하지 못한 호출은 예약을 그대로 남긴다.

요청 하나의 상한은 프롬프트 문자 수를 입력 토큰 상한으로, 요청에 건 출력 토큰 상한을 출력으로 잡아
적용 단가로 계산한다. 과금 항목은 제공자 정의를 따르며 합계 필드(`totalTokenCount`)를 다시 더하지 않는다.
사고 토큰은 출력 단가로 센다.
